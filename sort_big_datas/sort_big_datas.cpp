...
//题：有10个文件，每个文件1G，每个文件的每行存放的都是用户的query（请自己随机产生），每个文件的query都可能重复。要求你按照query的频度排序。
#include <fstream>
#include <iostream>
...
#define STD_STRING   //by lizheng define
...
public:
	explicit LocalSink(int idx, int nbuckets)
	{
		...
		sprintf(buf, "shard-%05d-of-%05d", idx, nbuckets);
		out.open(buf);
	}
...
void reduce(const int nbuckets)
{
	for (int i = 0; i < nbuckets; ++i)
	{
		...
		sprintf(buf, "count-%05d-of-%05d", i, nbuckets);
